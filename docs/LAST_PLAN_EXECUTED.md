...
# Plan — `github.com/veltylabs/editorial`: a list stops claiming it has columns
...
**Files:** `model_orm.go` (10 list types).
...
3. Confirm the diff contains **only** removals of the two stub methods —
   10 `Schema()` and 10 `Pointers()` lines — and nothing else. If
...
| Stage | Files | Action |
|---|---|---|
| 1 | `model_orm.go` | regenerate with `ormc`; 10 stub pairs disappear |
